...
window.addEventListener('load', () => {
    ...
    document.getElementById('signupBtn').addEventListener('click', async () => {
        ...
        try {
            ...
            await set(ref(db, 'users/' + user.uid), {
                ...
                streak: 0, lastCheckIn: null, lastSpin: null,
                tasks: { telegram: false, twitter: false, video: false }
            });
    ...
    document.getElementById('spinBtn').addEventListener('click', async () => {
        ...
        if (snapshot.exists()) {
            ...
            if (now - lastSpin < hours24) {
                window.showToast("Come back tomorrow to spin again!", "fa-circle-info");
                return;
            }

            const rewards = [1, 5, 10, 25, 50, 0]; // 0 is "Try Again"
            const winAmount = rewards[Math.floor(Math.random() * rewards.length)];

            let updates = { lastSpin: now };
            if (winAmount > 0) {
                updates.balance = (parseFloat(data.balance) || 0) + winAmount;
                updates.lifetimeEarned = (parseFloat(data.lifetimeEarned) || 0) + winAmount;
            }

            await update(userRef, updates);
            if (winAmount > 0) window.showToast(`🎉 You won ${winAmount} NEX!`);
            else window.showToast("Try again tomorrow!", "fa-face-frown");
        }
    });

    // 7. Deposit & Withdrawal
    document.getElementById('openDepositBtn').addEventListener('click', () => document.getElementById('depositModal').style.display = 'flex');
    document.getElementById('closeDepositModal').addEventListener('click', () => document.getElementById('depositModal').style.display = 'none');
    document.getElementById('submitDeposit').addEventListener('click', async () => {
        const user = auth.currentUser; if (!user) return;
        const method = document.getElementById('depositMethod').value; const txId = document.getElementById('depositTxId').value; const amountUsd = parseFloat(document.getElementById('depositAmountUsd').value);
        if (!txId || !amountUsd) { window.showToast("Please fill all fields", "fa-triangle-exclamation"); return; }

        const newDepRef = push(ref(db, 'deposits'));
        await set(newDepRef, { userId: user.uid, email: user.email, method, txId, amountUsd, status: 'pending', requestedAt: new Date().toISOString() });
        window.showToast("Deposit submitted! Awaiting confirmation.");
        document.getElementById('depositModal').style.display = 'none';
        document.getElementById('depositTxId').value = ''; document.getElementById('depositAmountUsd').value = '';
    });
    ...
    // 8. Multi-Level Referral Claim Logic
    window.claimReferrals = async () => {
        const user = auth.currentUser; if (!user) return;
        const btn = document.getElementById('claimRefBtn'); btn.innerText = "Checking..."; btn.disabled = true;
        try {
            const mySnap = await get(ref(db, 'users/' + user.uid)); if (!mySnap.exists()) return;
            const myData = mySnap.val(); const myRefCode = myData.myRefCode;
            const allUsersSnap = await get(ref(db, 'users')); const claimedSnap = await get(ref(db, 'referralsClaimed/' + user.uid));
            const allUsers = allUsersSnap.val() || {}; const claimed = claimedSnap.val() || {};
            let l1 = [], l2 = [], l3 = [];
            for (let uid in allUsers) { if (allUsers[uid].referredBy === myRefCode) l1.push(uid); }
            for (let l1_uid of l1) { let l1_refCode = allUsers[l1_uid].myRefCode; for (let uid in allUsers) { if (allUsers[uid].referredBy === l1_refCode) { l2.push(uid); let l2_refCode = allUsers[uid].myRefCode; for (let uid2 in allUsers) { if (allUsers[uid2].referredBy === l2_refCode) l3.push(uid2); } } } }
            let totalEarned = 0; let updates = {}; let newClaimsCount = 0;
            ...
            if (totalEarned > 0) {
                ...
                await update(ref(db), updates); window.showToast(`Claimed ${totalEarned} NEX from ${newClaimsCount} referrals!`);
            } else { window.showToast("No new referrals to claim", "fa-circle-info"); }
        } catch(e) { window.showToast("Error claiming", "fa-triangle-exclamation"); } finally { btn.innerText = "Claim Referral Bonuses"; btn.disabled = false; }
    };

    // 9. Auth State & Real-time Updates
    onAuthStateChanged(auth, async (user) => {
        if (user) {
            ...
            onValue(allUsersRef, (snapshot) => {
                let refCount = 0; let usersArray = [];
                if (snapshot.exists()) {
                    const users = snapshot.val();
                    for (let uid in users) { if (users[uid].referredBy === user.uid.substring(0, 6).toUpperCase()) refCount++; usersArray.push(users[uid]); }
                }
                ...
                let top5 = usersArray.slice(0, 5); let lbHtml = '';
            ...
            miningInterval = setInterval(() => { pendingNex += 0.001; document.getElementById('pendingBalance').innerText = `${pendingNex.toFixed(4)} NEX`; }, 1000);
    ...
    function renderInvestments(investments) {
        const listDiv = document.getElementById('investmentsList'); const countDiv = document.getElementById('activeInvestmentsCount');
        ...
        for (let name in investments) {
            count++; const inv = investments[name];
